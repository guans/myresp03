...
*/
		...
		Place p[]=new Place[2];
		p[0]=new Place("78-2B-CB-DF-93-BE",2.34,3.45,5.67);
		p[1]=new Place("57-7C-DG-ET-34-SD",8.67,4.88,7.86);
		sender.sendMessage(DoLocateAll(p.length, p));
	...
	// 发送房间所有人坐标 
		public static Message DoLocateAll(int num, Place p[]) /* web端 */
		throws Exception {
			SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");// 设置日期格式
			Message t = new Message();
			t.setRecipient(Setting.WEB_ENGINE_ID);
			...
			t.setHeader("length", "16");
			t.setHeader("timeid", df.format(new Date()));
			t.setHeader("type", "locateall");
			t.setHeader("from", "server");
			t.setHeader("to", "web");
			t.setHeader("num", String.valueOf(num)); // 返回的节点个数
			// 消息 （节点名+(x,y,z))
			byte[] content = new byte[(32 + 24) * num];
			for (int i = 0; i < num; i++) {
				for (int j = 0; j < p[i].name.getBytes().length; j++) {
					content[i*(24+32)+j] = p[i].name.getBytes()[j];
				}
